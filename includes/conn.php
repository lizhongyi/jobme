		...
		function get_realid($id){
			...
			function get_stage_nums($gid,$sid){
			 ...
			 function get_clientname($id){
				         ...
				         if(!$id) return false;
						 $name=M('Client')->where('id='.$id)->getField('company_name');
				          return $name;
				 }
			...
			function get_user_face($uid){
				         ...
				         if($uid){
							   ...
							   $face=M('User')->where('uid='.$uid)->getField('face');
							   if(!$face){
...
function _U2_Utf8_Gb($_C){
		...
		function company_logo($file){
			
			       
				   if(!$file){
					     
						 $url="/Public/img/nopic.jpg";
					   
					   }else{
						   
						   $url=C('OA_URL')."/Uploads/".$file;
