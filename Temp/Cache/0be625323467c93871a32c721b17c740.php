...
<h3 class="title-big">我的保存的搜索条件</h3>
...
<?php if(isset($getList)): ?><form method="post" action="<?php echo U("Sr/doCommand");?>">

<table width="100%" class="table job-list">
<tr>
  <th colspan="2">序号</th>
  <th>标题</th>
  <th>发布日期</th>
...
<td width="62" align="center" ><?php echo ($key+1); ?></td>
<td width="26" align="left"><input  style="width:20px;" type="checkbox" name="id[]" id="all3" class="piliang_box" value="<?php echo ($vo["id"]); ?>" /></td>
<td width="529" height="22"><a href="/Job?<?php echo ($vo['url']); ?>" title="<?php echo ($vo['title']); ?>"><?php echo ($vo['title']); ?></a></td>
<td width="165"><p><?php echo (date('Y-m-d',$vo["create_time"])); ?></p></td>
<td width="51" align="center"><a href="/Sr/delete/id/<?php echo ($vo['id']); ?>">删除</a></td><?php endforeach; endif; else: echo "" ;endif; ?>
<tr>
<th height="27" colspan="2" class=""><div class="btn-group"> <span class="btn btn-mini"><input name="operate" id="operate" value="delete" type="hidden"/>  <input style="width:20px;"  type="checkbox" name="all2" id="chkall" onclick="checkAll(this.form, 'id')" /></span><button class="btn" style="width:30px; height:25px;"><i class="icon-trash"></i>　</button></div></th>
<th colspan="4" >
