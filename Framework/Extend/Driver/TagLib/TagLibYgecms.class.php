...
class TagLibYgecms extends TagLib{



// 标签定义
 protected $tags   =  array(
        'getData'  => array('attr'=>'model,id,name,sql,mod,limit,order,where'),
		'getItem'  => array('attr'=>'model,where,sql')
        );
...
if(empty($sql)){
$parseStr  .= ' if(!isset('.$name.')) :'.$name.' = $'.$model.'Dao->Where($condition)->Order($order)->Limit($limit)->select();endif;';
}else{
