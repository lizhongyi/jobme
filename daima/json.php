<?php    
        ...
        if( $act      ==  'fuze'){
			...
			foreach ($user_arr as $k=>$v){
					   ...
					   $user_arr[$k]['pinyin']=$info['realname']==''? '' :Pinyin($info['realname'],$_Code='UTF-8');
			...
			if($nid) {
				...
				$where="and pid =".$uid." and consultant = 0 and uid not in($nid) and pid <> 0 or uid=".$uid." and consultant = 0 and uid not in($nid) and pid <> 0";
		   ...
		   $row=$db->getRow($sql);	
		    echo JSON($row);
		     ...
		     echo JSON($user_info);
			  ...
			  $sql="SELECT  A.*,B.realname 
                                     FROM dev_members AS A, dev_members_info AS B
                                     WHERE A.username =  '$uname' and A.password='$pass' and A.consultant = 3 
                                     AND A.uid = B.uid
                                     LIMIT 0 , 1";
